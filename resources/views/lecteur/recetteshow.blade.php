...
<div class="min-h-screen bg-white py-12">
    <div class="max-w-4xl mx-auto px-6">
        <article class="prose prose-lg max-w-none">
            ...
            <h1 class="text-4xl font-bold text-gray-800 mb-4">{{ $recette->titre }}</h1>
            
            <div class="flex items-center gap-4 text-gray-600 mb-8">
                @if($recette->auteur)
                    <span><i class="fas fa-user mr-2"></i>{{ $recette->auteur->nom }}</span>
                @endif
                <span><i class="fas fa-clock mr-2"></i>{{ $recette->reading_time }} min de lecture</span>
                @if($recette->region)
                    <span><i class="fas fa-map-marker-alt mr-2"></i>{{ $recette->region->nom_region }}</span>
                @endif
            </div>
        ...
        <div class="mt-12 pt-8 border-t border-gray-200">
            ...
            @if(Auth::check() && $aPaye)
                <div class="bg-white border border-gray-200 rounded-xl p-8 shadow-sm">
                    ...
                    <form action="{{ route('lecteur.commentaire.store', $recette->id) }}" method="POST">
                        @csrf
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <textarea name="texte" 
                                          ...
                                          placeholder="Qu'avez-vous pensé de cette recette ? Partagez votre avis..."
                                          required></textarea>
                            </div>
            ...
            @elseif(Auth::check() && !$aPaye)
                <div class="bg-amber-50 border border-amber-200 rounded-xl p-6 text-center">
                    ...
                    <p class="text-amber-700 mb-4">
                        Vous devez avoir acheté cette recette pour pouvoir la commenter.
                    </p>
                    <a href="{{ route('paiement.recette.initier', $recette->id) }}"
            ...
            @else
                <div class="bg-amber-50 border border-amber-200 rounded-xl p-8 text-center">
                    ...
                    <p class="text-amber-700 mb-6 max-w-md mx-auto">
                        Pour partager votre avis sur cette recette, vous devez être connecté à votre compte lecteur.
                    </p>
